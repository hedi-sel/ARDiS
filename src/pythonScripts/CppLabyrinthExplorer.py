...
def RecordResults(name, dt=1e-2, verbose=True):
    if verbose:
...
def CppExploreLabyrinth(name, reaction=5, output=OutputType.NONE, return_item=ReturnType.SUCCESS, verbose=True, dt=1e-2, epsilon=1e-3, max_time=3, plot_dt=1e-1, guess=float(0), use_system=True,
                        threashold=0.9, UseCpp=True):
    ...
    if True:  # Load Mesh and matrices from file
        ...
        fillVal = 0.001
        U = np.full(n, fillVal)
        StartZone = TriangleZone(0, 0, 10, 0, 5, 0.2)
        # nTests = 10
        # angles = np.linspace(0, math.pi/2, nTests+1)
        # for i in range(0, nTests):
        #     FillZone(U, Mesh,
        #              TriangleZone(0, 0, 3*math.cos(angles[i]), 3*math.sin(
    ...
    if DoRecordResult:
        RecordResults(name, dt, verbose=verbose)

    if (DoPlot):
        PrintLabyrinth(name, verbose=verbose, plotEvery=int(plot_dt/dt))
