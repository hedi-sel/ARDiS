...
# os.system("./wolframScripts/ImageToMatrix.wls maze_e-2_e-3")
...
# size = "100"
#  os.system("./wolframScripts/MazeGenerator.wls maze_"+size+" "+size)
...
for step in stepSizes:
    ...
    for j in range(0, Nexpr):
        perf_times = ExploreLabyrinth("precision="+step, output=OutputType.PLOT, return_item=ReturnType.LOADING_COMPUTATION_TIME, storeEvery=1, verbose=True, fastCalculation=False,
                                      max_time=10, diffusion=1, reaction=1, dt=1e-1, drain=1e-8, epsilon=1e-3, plot_dt=1, startZone=dg.RectangleZone(0, 0, 500, 10))
...
# step = "0.2"
# os.system("./wolframScripts/ImageToMatrixControlPrecision.wls precision=" + step + " " + step)
